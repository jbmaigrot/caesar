...
public class ProgrammableObjectsData : MonoBehaviour
{
    ...
    void OnMouseDown()
    {
        if (isHackable && (this.GetComponent<Collider>().ClosestPoint(client.characters[client.playerIndex].transform.position) - client.characters[client.playerIndex].transform.position).magnitude < 5 &&!hackInterface.GetComponent<CanvasGroup>().blocksRaycasts)
        {
